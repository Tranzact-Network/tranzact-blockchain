...
from tranzact.daemon.keychain_proxy import (
    KeychainProxy,
...
from tranzact.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from tranzact.protocols import wallet_protocol
from tranzact.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
...
from tranzact.server.node_discovery import WalletPeers
...
from tranzact.util.ints import uint32, uint128
...
from tranzact.wallet.settings.settings_objects import BackupInitialized
from tranzact.wallet.transaction_record import TransactionRecord
from tranzact.wallet.util.backup_utils import open_backup_file
from tranzact.wallet.util.wallet_types import WalletType
from tranzact.wallet.wallet_action import WalletAction
from tranzact.wallet.wallet_blockchain import ReceiveBlockResult
from tranzact.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    server: Optional[TranzactServer]
    log: logging.Logger
    ...
    async def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        try:
            ...
            self.log.warning("No keys present. Create keys with the UI, or with the 'tranzact keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSTranzactConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSTranzactConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSTranzactConnection):
        if self.wallet_state_manager is None:
            return
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSTranzactConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(
        self, peer: WSTranzactConnection, block_i, additions: Optional[List[bytes32]], get_all_additions: bool = False
    ) -> Optional[List[Coin]]:
    ...
    async def get_removals(
        self, peer: WSTranzactConnection, block_i, additions, removals, request_all_removals=False
    ) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
...
async def wallet_next_block_check(
    peer: WSTranzactConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:

...
from tranzact.wallet.puzzles.puzzle_utils import (
...
from tranzact.wallet.secret_key_store import SecretKeyStore
from tranzact.wallet.sign_coin_spends import sign_coin_spends
...
class Wallet:
    ...
    async def create_spend_bundle_relative_tranzact(self, tranzact_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if tranzact_amount < 0:
            utxos = await self.select_coins(abs(tranzact_amount), exclude)
        else:
        ...
        tranzact_amount = spend_value + tranzact_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": tranzact_amount}]

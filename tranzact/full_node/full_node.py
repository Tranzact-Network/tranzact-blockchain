...
import tranzact.server.ws_connection as ws  # lgtm [py/import-and-import-from]
...
from tranzact.consensus.blockchain import Blockchain, ReceiveBlockResult
...
from tranzact.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from tranzact.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from tranzact.consensus.multiprocess_validation import PreValidationResult
from tranzact.consensus.pot_iterations import calculate_sp_iters
from tranzact.full_node.block_store import BlockStore
from tranzact.full_node.lock_queue import LockQueue, LockClient
...
from tranzact.full_node.full_node_store import FullNodeStore, FullNodeStorePeakResult
from tranzact.full_node.hint_store import HintStore
from tranzact.full_node.mempool_manager import MempoolManager
from tranzact.full_node.signage_point import SignagePoint
from tranzact.full_node.sync_store import SyncStore
from tranzact.full_node.weight_proof import WeightProofHandler
from tranzact.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from tranzact.protocols.full_node_protocol import (
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
    RespondSignagePoint,
)
from tranzact.protocols.protocol_message_types import ProtocolMessageTypes
from tranzact.protocols.wallet_protocol import CoinState, CoinStateUpdate
from tranzact.server.node_discovery import FullNodePeers
from tranzact.server.outbound_message import Message, NodeType, make_msg
from tranzact.server.server import TranzactServer
...
from tranzact.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
...
from tranzact.types.transaction_queue_entry import TransactionQueueEntry
from tranzact.types.unfinished_block import UnfinishedBlock
from tranzact.util import cached_bls
from tranzact.util.bech32m import encode_puzzle_hash
from tranzact.util.check_fork_next_block import check_fork_next_block
from tranzact.util.condition_tools import pkm_pairs
from tranzact.util.db_wrapper import DBWrapper
from tranzact.util.errors import ConsensusError, Err, ValidationError
from tranzact.util.ints import uint8, uint32, uint64, uint128
from tranzact.util.path import mkdir, path_from_root
...
class FullNode:
    ...
    def set_server(self, server: TranzactServer):
        self.server = server
        dns_servers = []
        try:
        ...
        elif self.config["port"] == 8655:
            # If `dns_servers` misses from the `config`, hardcode it if we're running mainnet.
            dns_servers.append("dns-introducer.tranzact.cash")
        try:
    ...
    async def short_sync_batch(self, peer: ws.WSTranzactConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSTranzactConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSTranzactConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSTranzactConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSTranzactConnection):
        """
    ...
    def on_disconnect(self, connection: ws.WSTranzactConnection):
    ...
    def remove_subscriptions(self, peer: ws.WSTranzactConnection):
    ...
    async def sync_from_fork_point(
        ...
        async def fetch_block_batches(batch_queue, peers_with_peak: List[ws.WSTranzactConnection]):
            try:
        ...
        batch_queue: asyncio.Queue[Tuple[ws.WSTranzactConnection, List[FullBlock]]] = asyncio.Queue(
    ...
    async def update_wallets(
        ...
        for peer, changes in changes_for_peer.items():
            if peer not in self.server.all_connections:
                continue
            ws_peer: ws.WSTranzactConnection = self.server.all_connections[peer]
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSTranzactConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        if farmed_block is True:
            ...
            self.log.info(
                ...
                f"Pool pk {encode_puzzle_hash(block.foliage.foliage_block_data.pool_target.puzzle_hash, 'trz')}, "
                f"validation time: {validation_time:0.4f} seconds, {pre_validation_log}"
                f"cost: {block.transactions_info.cost if block.transactions_info else 'None'}"
                f"{percent_full_str}"
            )
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSTranzactConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSTranzactConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSTranzactConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSTranzactConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSTranzactConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
...
async def node_next_block_check(
    peer: ws.WSTranzactConnection, potential_peek: uint32, blockchain: BlockchainInterface
) -> bool:

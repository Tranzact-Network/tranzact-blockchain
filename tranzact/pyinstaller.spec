...
ROOT = pathlib.Path(importlib.import_module("tranzact").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `tranzact` file name (which is the executable)
    and the `tranzact` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `tranzact` directory, since the names collide.
    ...
    for data in analysis.datas:
        if str(data[0]).startswith("tranzact/"):
            zipped.append(data)
        else:
            datas.append(data)
...
version_data = copy_metadata(get_distribution("tranzact-blockchain"))[0]
...
# TODO: collapse all these entry points into one `tranzact_exec` entrypoint that accepts the server as a parameter

entry_points = ["tranzact.cmds.tranzact"] + [f"tranzact.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "tranzact.util.bip39"])

if THIS_IS_WINDOWS:
    tranzact_mod = importlib.import_module("tranzact")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/tranzact/util/english.txt", "tranzact/util"))
datas.append((f"{ROOT}/tranzact/util/initial-config.yaml", "tranzact/util"))
datas.append((f"{ROOT}/tranzact/wallet/puzzles/*.hex", "tranzact/wallet/puzzles"))
datas.append((f"{ROOT}/tranzact/ssl/*", "tranzact/ssl"))
...
add_binary("tranzact", f"{ROOT}/tranzact/cmds/tranzact.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/tranzact/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/tranzact/server/start_{server}.py", COLLECT_ARGS)

...
from tranzact.cmds.configure import configure_cmd
from tranzact.cmds.farm import farm_cmd
from tranzact.cmds.init import init_cmd
from tranzact.cmds.keys import keys_cmd
from tranzact.cmds.netspace import netspace_cmd
from tranzact.cmds.passphrase import passphrase_cmd
from tranzact.cmds.plots import plots_cmd
from tranzact.cmds.show import show_cmd
from tranzact.cmds.start import start_cmd
from tranzact.cmds.stop import stop_cmd
from tranzact.cmds.wallet import wallet_cmd
from tranzact.cmds.plotnft import plotnft_cmd
from tranzact.cmds.plotters import plotters_cmd
from tranzact.util.default_root import DEFAULT_KEYS_ROOT_PATH, DEFAULT_ROOT_PATH
from tranzact.util.keychain import set_keys_root_path, supports_keyring_passphrase
from tranzact.util.ssl_check import check_ssl
...
@click.group(
    help=f"\n  Manage tranzact blockchain infrastructure ({__version__})\n",
    epilog="Try 'tranzact start node', 'tranzact netspace -d 192', or 'tranzact show -s'",
...
def cli(
    ...
    if passphrase_file is not None:
        from tranzact.cmds.passphrase_funcs import cache_passphrase, read_passphrase_from_file
...
if not supports_keyring_passphrase():
    from tranzact.cmds.passphrase_funcs import remove_passphrase_options_from_cmd

    # TODO: Remove once keyring passphrase management is rolled out to all platforms
    remove_passphrase_options_from_cmd(cli)


@cli.command("version", short_help="Show tranzact version")
def version_cmd() -> None:
    print(__version__)


@cli.command("run_daemon", short_help="Runs tranzact daemon")
@click.option(
    "--wait-for-unlock",
    help="If the keyring is passphrase-protected, the daemon will wait for an unlock command before accessing keys",
    default=False,
    is_flag=True,
    hidden=True,  # --wait-for-unlock is only set when launched by tranzact start <service>
)
@click.pass_context
def run_daemon_cmd(ctx: click.Context, wait_for_unlock: bool) -> None:
    import asyncio
    from tranzact.daemon.server import async_run_daemon

...
from tranzact.cmds.units import units
from tranzact.consensus.block_record import BlockRecord
from tranzact.rpc.farmer_rpc_client import FarmerRpcClient
from tranzact.rpc.full_node_rpc_client import FullNodeRpcClient
from tranzact.rpc.wallet_rpc_client import WalletRpcClient
...
from tranzact.util.misc import format_bytes
from tranzact.util.misc import format_minutes
from tranzact.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total tranzact farmed: {amounts['farmed_amount'] / units['tranzact']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['tranzact']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['tranzact']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'tranzact start wallet' and 'tranzact wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'tranzact wallet show'")
    else:
        print("Note: log into your key using 'tranzact wallet show' to see rewards for each key")

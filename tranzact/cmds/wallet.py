...
@wallet_cmd.command("send", short_help="Send tranzact to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much tranzact to send, in TRZ", type=str, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in TRZ",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the TRZ", type=str, required=True)
@click.option(
...
def add_chia_pool_address_cmd(wallet_rpc_port: Optional[int], id, fingerprint: int, launcher_id:str, pool_contract_address:str) -> None:
    extra_params = {"id": id, "launcher_id": launcher_id, "pool_contract_address": pool_contract_address}
    import asyncio
    from .wallet_funcs import execute_with_wallet, add_chia_pool_address

    asyncio.run(execute_with_wallet(wallet_rpc_port, fingerprint, extra_params, add_chia_pool_address))

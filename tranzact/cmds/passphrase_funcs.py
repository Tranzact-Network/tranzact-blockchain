...
from tranzact.daemon.client import acquire_connection_to_daemon
from tranzact.util.keychain import Keychain, obtain_current_passphrase, supports_os_passphrase_storage
...
from tranzact.util.ws_message import WsRpcMessage
    ...
    try:
        if supports_os_passphrase_storage():
            ...
            print(
                ...
                "Other Tranzact processes will be able to access your keys without prompting for your passphrase."
            )
...
def initialize_passphrase() -> None:
    if Keychain.has_master_passphrase():
        ...
        print("\nUse 'tranzact passphrase set' or 'tranzact passphrase remove' to update or remove your passphrase")
        sys.exit(1)
...
async def async_update_daemon_passphrase_cache_if_running(root_path: Path) -> None:
    from tranzact.daemon.client import connect_to_daemon_and_validate

...
from tranzact.ssl.create_ssl import (
    ensure_ssl_dirs,
    generate_ca_signed_cert,
    get_tranzact_ca_crt_key,
    make_ca_cert,
    write_ssl_cert_and_key,
)
from tranzact.util.bech32m import encode_puzzle_hash
from tranzact.util.config import (
    create_default_tranzact_config,
    initial_config_file,
    load_config,
    save_config,
...
from tranzact.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path, keychain: Optional[Keychain] = None) -> None:
    ...
    if len(all_sks) == 0:
        print("No keys are present in the keychain. Generate them with 'tranzact keys generate'")
        return None

    config: Dict = load_config(new_root, "config.yaml")
    pool_child_pubkeys = [master_sk_to_pool_sk(sk).get_g1() for sk, _ in all_sks]
    all_targets = []
    stop_searching_for_farmer = "trz_target_address" not in config["farmer"]
    stop_searching_for_pool = "trz_target_address" not in config["pool"]
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("trz_target_address"):
    ...
    if "trz_target_address" not in config["farmer"]:
        ...
        print(
            ...
            f"{config['farmer']['trz_target_address']} with {all_targets[0]}"
        )
...
def create_all_ssl(root_path: Path):
    ...
    tranzact_ca_crt, tranzact_ca_key = get_tranzact_ca_crt_key()
    tranzact_ca_crt_path = ca_dir / "tranzact_ca.crt"
    tranzact_ca_key_path = ca_dir / "tranzact_ca.key"
    write_ssl_cert_and_key(tranzact_ca_crt_path, tranzact_ca_crt, tranzact_ca_key_path, tranzact_ca_key)
    ...
    tranzact_ca_crt, tranzact_ca_key = get_tranzact_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, tranzact_ca_crt, tranzact_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path, fix_ssl_permissions: bool = False):
    if create_certs is not None:
        if root_path.exists():
            ...
            if tranzact_init(root_path, fix_ssl_permissions=fix_ssl_permissions) == 0 and root_path.exists():
                return init(create_certs, root_path, fix_ssl_permissions)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return tranzact_init(root_path, fix_ssl_permissions=fix_ssl_permissions)


def tranzact_version_number() -> Tuple[str, str, str, str]:
...
def tranzact_minor_release_number():
    res = int(tranzact_version_number()[2])
    print(f"Install release number: {res}")
    return res


def tranzact_full_version_str() -> str:
    major, minor, patch, dev = tranzact_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def tranzact_init(root_path: Path, *, should_check_keys: bool = True, fix_ssl_permissions: bool = False):
    """
    ...
    """
    if os.environ.get("TRANZACT_ROOT", None) is not None:
        print(
            f"warning, your TRANZACT_ROOT is set to {os.environ['TRANZACT_ROOT']}. "
            f"Please unset the environment variable and run tranzact init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Tranzact directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if TRANZACT_ROOT is set, or if user has run tranzact init twice
        # before a new update.
    ...
    if should_check_keys:
        check_keys(root_path)
    print("")
    print("To see your keys, run 'tranzact keys show --show-mnemonic-seed'")

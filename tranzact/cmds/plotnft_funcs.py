...
from tranzact.cmds.wallet_funcs import print_balance, wallet_coin_unit
from tranzact.pools.pool_wallet_info import PoolWalletInfo, PoolSingletonState
from tranzact.protocols.pool_protocol import POOL_PROTOCOL_VERSION
...
from tranzact.util.ints import uint16, uint32, uint64
...
async def create(args: dict, wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    fee_mojos = uint64(int(fee * units["tranzact"]))
        ...
        try:
            ...
            while time.time() - start < 10:
                ...
                if len(tx.sent_to) > 0:
                    ...
                    print(f"Do tranzact wallet get_transaction -f {fingerprint} -tx 0x{tx_record.name} to get status")
                    return None
        except Exception as e:

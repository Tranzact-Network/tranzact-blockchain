...
from tranzact.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
from tranzact.protocols.protocol_message_types import ProtocolMessageTypes
from tranzact.server.outbound_message import NodeType, make_msg
...
from tranzact.util.bech32m import decode_puzzle_hash, encode_puzzle_hash
from tranzact.util.byte_types import hexstr_to_bytes
from tranzact.util.ints import uint32, uint64
from tranzact.util.keychain import KeyringIsLocked, bytes_to_mnemonic, generate_mnemonic
...
from tranzact.wallet.derive_keys import master_sk_to_singleton_owner_sk
from tranzact.wallet.rl_wallet.rl_wallet import RLWallet
from tranzact.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from tranzact.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from tranzact.wallet.util.trade_utils import trade_record_to_dict
...
from tranzact.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "tranzact_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "tranzact_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("trz_target_address")
        pool_target = config["pool"].get("trz_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from tranzact.pools.pool_wallet_info import initial_pool_state_from_dict

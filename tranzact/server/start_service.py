...
from tranzact.daemon.server import singleton, service_launch_lock_path
from tranzact.server.ssl_context import tranzact_ssl_ca_paths, private_ssl_ca_paths
...
class Service:
    def __init__(
        ...
        proctitle_name = f"tranzact_{service_name}"
        ...
        tranzact_ca_crt, tranzact_ca_key = tranzact_ssl_ca_paths(root_path, self.config)
        inbound_rlp = self.config.get("inbound_rate_limit_percent")
        outbound_rlp = self.config.get("outbound_rate_limit_percent")
        assert inbound_rlp and outbound_rlp
        self._server = TranzactServer(
            advertised_port,
            node,
            peer_api,
            node_type,
            ...
            (tranzact_ca_crt, tranzact_ca_key),
    ...
    async def wait_closed(self) -> None:
        ...
        self._log.info("Waiting for TranzactServer to be closed")

...
from tranzact.cmds.init_funcs import check_keys, tranzact_init
from tranzact.cmds.passphrase_funcs import default_passphrase, using_default_passphrase
from tranzact.daemon.keychain_server import KeychainServer, keychain_commands
from tranzact.daemon.windows_signal import kill
from tranzact.plotters.plotters import get_available_plotters
from tranzact.plotting.util import add_plot_directory
from tranzact.server.server import ssl_context_for_root, ssl_context_for_server
from tranzact.ssl.create_ssl import get_mozilla_ca_crt
from tranzact.util.tranzact_logging import initialize_logging
from tranzact.util.config import load_config
from tranzact.util.json_util import dict_to_json_str
...
from tranzact.util.setproctitle import setproctitle
from tranzact.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Tranzact.")
    quit()
...
service_plotter = "tranzact_plotter"
...
if getattr(sys, "frozen", False):
    name_map = {
        "tranzact": "tranzact",
        "tranzact_wallet": "start_wallet",
        "tranzact_full_node": "start_full_node",
        "tranzact_harvester": "start_harvester",
        "tranzact_farmer": "start_farmer",
        "tranzact_introducer": "start_introducer",
        "tranzact_timelord": "start_timelord",
        "tranzact_timelord_launcher": "timelord_launcher",
        "tranzact_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("tranzact daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered TRANZACT_ROOT
    os.environ["TRANZACT_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with TRANZACT_ROOT: {os.environ['TRANZACT_ROOT']}")
    ...
    if service_command == "tranzact_full_node_simulator":
        # Set the -D/--connect_to_daemon flag to signify that the child should connect
        # to the daemon to access the keychain
...
async def async_run_daemon(root_path: Path, wait_for_unlock: bool = False) -> int:
    # When wait_for_unlock is true, we want to skip the check_keys() call in tranzact_init
    # since it might be necessary to wait for the GUI to unlock the keyring first.
    tranzact_init(root_path, should_check_keys=(not wait_for_unlock))
    config = load_config(root_path, "config.yaml")
    setproctitle("tranzact_daemon")
...
def main(argv) -> int:
    from tranzact.util.default_root import DEFAULT_ROOT_PATH

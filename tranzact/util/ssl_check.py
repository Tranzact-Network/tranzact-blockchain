...
import stat
import sys
from tranzact.util.config import load_config, traverse_dict
from tranzact.util.permissions import octal_mode_string, verify_file_permissions
...
def check_ssl(root_path: Path) -> None:
    ...
    if len(invalid_files):
        ...
        print("Run `tranzact init --fix-ssl-permissions` to fix issues.")

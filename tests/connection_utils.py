...
async def disconnect_all_and_reconnect(server: TranzactServer, reconnect_to: TranzactServer) -> bool:
...
async def add_dummy_connection(server: TranzactServer, dummy_port: int) -> Tuple[asyncio.Queue, bytes32]:
    ...
    generate_ca_signed_cert(
        server.tranzact_ca_crt_path.read_bytes(), server.tranzact_ca_key_path.read_bytes(), dummy_crt_path, dummy_key_path
    )
    ssl_context = ssl_context_for_client(
        server.tranzact_ca_crt_path, server.tranzact_ca_key_path, dummy_crt_path, dummy_key_path
    )
...
async def connect_and_get_peer(server_1: TranzactServer, server_2: TranzactServer) -> WSTranzactConnection:
    """

...
from tranzact.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from tranzact.full_node.full_node_api import FullNodeAPI
from tranzact.server.start_farmer import service_kwargs_for_farmer
from tranzact.server.start_full_node import service_kwargs_for_full_node
from tranzact.server.start_harvester import service_kwargs_for_harvester
from tranzact.server.start_introducer import service_kwargs_for_introducer
from tranzact.server.start_service import Service
from tranzact.server.start_timelord import service_kwargs_for_timelord
from tranzact.server.start_wallet import service_kwargs_for_wallet
from tranzact.simulator.start_simulator import service_kwargs_for_full_node_simulator
from tranzact.timelord.timelord_launcher import kill_processes, spawn_process
...
from tranzact.util.ints import uint16, uint32
from tranzact.util.keychain import bytes_to_mnemonic
...
async def setup_farmer(
    ...
    config["trz_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "trz")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["trz_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "trz")
